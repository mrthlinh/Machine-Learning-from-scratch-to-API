...
with tf.Session() as session:
    ...
    #
    # print("Weight: ", tf.shape(weights.eval()) )
    # print("Bias: ", tf.shape(biases.eval()))
    # W = weights.eval()
    # b = biases.eval()
    step = 0
    ...
    #     acc, loss, onehot_pred = session.run([accuracy, cost, pred],feed_dict={x: symbols_in_keys, y: symbols_out_onehot})
    #     loss_total += loss
    #     acc_total += acc
    #
    ...
    while step < training_iters:
        ...
        acc, onehot_pred = session.run([accuracy, pred], \
                                                feed_dict={x: symbols_in_keys, y: symbols_out_onehot})
        # loss_total += loss
        acc_total += acc
        if (step+1) % display_step == 0:
